...
CI는 실제 Chromium에서 1920×1324 합성 영상 수신, 재시작, data channel 품질 명령과
수신 크기 변화를 검사한다. 합성 송신기는 iOS를 대신 검증하지 않는다.
macOS runner에서 Swift 테스트·Xcode 빌드·실제 framework 임베드·IPA 검사를 별도로 수행한다.
이 전달 시점에 새 커밋 업로드 및 GitHub Actions build20 성공을 확인한 것은 아니다.
