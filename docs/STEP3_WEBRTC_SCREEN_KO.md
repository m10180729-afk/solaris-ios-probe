# Solaris 0.3.2 build31 — 양방향 화면공유 설치와 확인

이 파일은 build31용입니다. 이전 IPA/HTML과 섞지 마세요.
검사 성공은 실제 기기의 60fps 보장을 의미하지 않습니다.

1. 소스 ZIP을 Solaris-0.3.2-build31-windows-1080p120-source 폴더에 모두 압축 해제합니다.
2. 해당 폴더에서 Git Bash로 bash UPLOAD_GIT_BASH.sh를 실행합니다.
3. 해당 커밋의 GitHub Actions에서 browser-test와 build가 모두 성공해야 합니다.
4. Solaris-0.3.2-build31-bidirectional-screen 아티팩트를 다운로드하고 압축 해제합니다.
5. Solaris-0.3.2-build31-resign.ipa를 AltStore로 설치합니다. 방송 확장을 제거하지 마세요.
6. iPad 앱에 0.3.2 (build 31)가 표시되는지 확인합니다.
7. iPad→Windows는 Solaris-Windows-0.3.2-build31.html을 Windows에서 엽니다.
...
build31의 iPad 송출 기본값은 최고화질 28Mbps 최소 요청과 60Mbps 상한입니다. 손실이 늘거나 끊기면
...
2. 송신 Windows에서 Solaris-Desktop-Share-build31.html을 엽니다.
...
6. 진단의 실제 값이 1920×1080, 100fps 이상이면 120fps 경로가 동작한 것입니다.
...
빌드 실패 시 Solaris-build31-xcode-evidence의 Xcode 로그가 필요합니다.
