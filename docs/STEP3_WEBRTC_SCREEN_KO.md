# Solaris 0.3.2 build25 — 설치와 확인

이 파일은 build25용입니다. 이전 build7/build19/build20/build23/build24 HTML이나 IPA와 섞지 마세요.
검사 성공은 실제 기기의 60fps 보장을 의미하지 않습니다.

1. 소스 ZIP을 Solaris-0.3.2-build25-quality-first-source 폴더에 모두 압축 해제합니다.
2. 해당 폴더에서 Git Bash로 bash UPLOAD_GIT_BASH.sh를 실행합니다.
3. 해당 커밋의 GitHub Actions에서 browser-test와 build가 모두 성공해야 합니다.
4. Solaris-0.3.2-build25-app-and-receiver 아티팩트를 다운로드하고 압축 해제합니다.
5. Solaris-0.3.2-build25-resign.ipa를 AltStore로 설치합니다. 방송 확장을 제거하지 마세요.
6. iPad 앱에 0.3.2 (build 25)가 표시되는지 확인합니다.
7. 같은 아티팩트의 Solaris-Windows-0.3.2-build25.html을 Windows에서 엽니다.
8. 기존 Supabase URL·Publishable key·방 ID solaristest1을 그대로 입력합니다.
9. 품질 ‘원본 1920급 · 60fps · 화질 우선’, 코덱 ‘H.264 하드웨어 · 권장’을 사용합니다.
...
build25는 화질 우선으로 20Mbps의 최소값과 60Mbps 상한을 요청합니다. WebRTC 혼잡
제어는 실제 네트워크 상태가 나쁘면 이보다 낮출 수 있으므로 진단의 실제 Mbps를 확인합니다.
...
- 네트워크: 가용 송신 Mbps·왕복 지연·손실률을 함께 기록합니다.
...
빌드 실패 시 Solaris-build25-xcode-evidence의 Xcode 로그가 필요합니다.
