...
build25는 빠른 화면 전환의 압축 뭉개짐을 줄이기 위해 화질 우선 20–60Mbps 정책과
750ms 수신 버퍼 요청, 손실률·RTT·가용 대역폭 진단을 추가합니다.

## build25 변경

- 검증된 H.264 VideoToolbox와 level 5.1 협상을 유지.
- RTP 최소 요청 20Mbps·최대 60Mbps, 화질 우선순위 4.0.
...
- 패킷 손실률·버퍼 지연·RTT·가용 송신 대역폭 진단 추가.
...
- IPA·HTML·아티팩트·앱 build 번호를 모두 25로 통일.

## 설치

소스를 압축 해제하고 해당 폴더에서 Git Bash로 bash UPLOAD_GIT_BASH.sh를 실행합니다.
GitHub Actions의 browser-test와 build 모두 성공 후
Solaris-0.3.2-build25-app-and-receiver를 다운로드합니다.
Solaris-0.3.2-build25-resign.ipa와 Solaris-Windows-0.3.2-build25.html을 함께 사용합니다.
